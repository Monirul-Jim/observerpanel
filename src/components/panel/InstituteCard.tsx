...
export function InstituteCard({ inst, periodIdx, onPress }: InstituteCardProps) {
  ...
  return (
    ...
    >
      <View className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-3.5 pl-[18px]">
        {/* Left accent bar */}
        <View
        ...
        <View className="flex-row items-start justify-between">
          <View className="min-w-0 flex-1">
            <View className="mb-1 flex-row items-center">
              ...
              <Text className="text-[10px] text-slate-500">{inst.code}</Text>
              <View className="ml-1.5 rounded bg-slate-100 px-1.5 py-0.5">
                <Text className="text-[9px] text-slate-600">{inst.type}</Text>
              </View>
            </View>
            <Text className="pr-2 text-sm font-bold leading-5 text-slate-900">{inst.name}</Text>
            <Text className="mt-0.5 text-[11px] text-slate-400">
              {inst.totalStudents} Students · {inst.upazila}, {inst.district}
            </Text>
          </View>
          <View className="shrink-0 items-end">
            <Text className="text-[10px] text-slate-500">{PERIOD_LABELS[periodIdx]}</Text>
            <Text
        ...
        {/* Stats row */}
        <View className="mt-2.5 flex-row border-y border-slate-100 py-2">
          {[
            ...
            <View key={i} className="flex-1 items-center">
              <Text className={`text-xs font-bold ${s.red ? 'text-red-500' : 'text-slate-700'}`}>
                {s.value}
              </Text>
              <Text className="mt-0.5 text-[10px] text-slate-400">{s.label}</Text>
            </View>
          ))}
        </View>
